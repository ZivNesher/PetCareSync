...
public class PetLogsActivity extends BaseActivity {
    ...
    private void loadLogs() {
        ...
        ref.orderByChild("petId").equalTo(petId).addValueEventListener(new ValueEventListener() {
            ...
            public void onCancelled(DatabaseError error) {
                Toast.makeText(PetLogsActivity.this, "Error loading logs", Toast.LENGTH_SHORT).show();
            }
        });
    }

    private void saveLog() {
        String note = logInput.getText().toString();
        if (note.isEmpty()) return;

        String id = UUID.randomUUID().toString();
        String date = new SimpleDateFormat("dd.MM.yyyy", Locale.getDefault()).format(new Date());
        HealthLog log = new HealthLog(id, petId, date, note, "owner");
